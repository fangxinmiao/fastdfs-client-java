...
public class ClientGlobal {
    ...
    public static void init(ClientProperties prop) throws Exception {
        g_connect_timeout = prop.getConnectTimeout() * 1000;
        g_network_timeout = prop.getNetworkTimeout() * 1000;
        g_charset = prop.getCharset();
        g_tracker_http_port = prop.getTrackerHttpPort();
        g_anti_steal_token = prop.isAntiStealToken();
        g_secret_key = prop.getSecretKey();

        String[] trackerServers = prop.getTrackerServers();
        if (trackerServers == null) {
            throw new RuntimeException("tracker_server not found");
        }
        InetSocketAddress[] addresses = new InetSocketAddress[trackerServers.length];
        for (int i = 0; i < trackerServers.length; i++) {
            String[] parts = trackerServers[i].split("\\:", 2);
            if (parts.length != 2) {
                throw new RuntimeException("tracker_server is invalid, " +
                        "the correct format is host:port");
            }
            addresses[i] = new InetSocketAddress(parts[0].trim(),
                    Integer.parseInt(parts[1].trim()));
        }
        g_tracker_group = new TrackerGroup(addresses);
    }
